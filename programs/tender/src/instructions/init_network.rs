...
pub fn handler<'info>(ctx: Context<InitNetwork>, proposal_collection_uri: String) -> Result<()> {
    ...
    (*ctx.accounts.network).council_collection_bump = *ctx.bumps.get("council_collection").unwrap();
    (*ctx.accounts.network).service_collection_bump = *ctx.bumps.get("service_collection").unwrap();
    (*ctx.accounts.network).proposal_collection_bump = *ctx.bumps.get("proposal_collection").unwrap();
    (*ctx.accounts.network).mint_bump = *ctx.bumps.get("network_mint").unwrap();
    (*ctx.accounts.network).bump = *ctx.bumps.get("network").unwrap();
